...
public class StudentController {
	...
	public ResponseEntity<?> addStudent(@RequestBody @Nullable DtoStudent dtoStudent) {
		try {
			...
			List<DtoStudent> existingStudents =
					daoStudent.findByNameAndSurname(dtoStudent.getName(), dtoStudent.getSurname());
			if ( Objects.nonNull(existingStudents) && !existingStudents.isEmpty() ) {
				ErrorDataResult<List<DtoStudent>> result = 
						new ErrorDataResult<List<DtoStudent>>(existingStudents, "Same students found with same name and surname");
