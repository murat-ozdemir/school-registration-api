...
public class CourseController {
	...
	public ResponseEntity<?> addCourse(@RequestBody @Nullable DtoCourse dtoCourse) {
		try {
			...
			List<DtoCourse> existingCourses =
					daoCourse.findByCategoryAndName(dtoCourse.getCategory(), dtoCourse.getCourseName());
			if ( Objects.nonNull(existingCourses) && !existingCourses.isEmpty() ) {
				ErrorDataResult<List<DtoCourse>> result = 
						new ErrorDataResult<List<DtoCourse>>(existingCourses, "Same courses found with same category and name");
